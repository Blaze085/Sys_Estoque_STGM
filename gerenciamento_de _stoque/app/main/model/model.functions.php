...
class relatorios{
    public function gerarRelatorioPersonalizado($startDate = null, $endDate = null, $categoria = null) {
        // Validate inputs
        ...
        $this->addHeader($pdf, "RELATÓRIO PERSONALIZADO");
    
        // Build dynamic SQL query
        $params = [];
        $whereClauses = [];
        if ($startDate && $endDate) {
            $whereClauses[] = "data_cadastro BETWEEN :startDate AND :endDate";
            $params['startDate'] = $startDate;
            $params['endDate'] = $endDate;
        }
        if ($categoria) {
            $whereClauses[] = "natureza = :categoria";
        ...
        $pdf->SetFillColor($this->corBranco[0], $this->corBranco[1], $this->corBranco[2]);
        $pdf->RoundedRect($startX, $startY, $cardWidth, $cardHeight, 8, 'F');
        $pdf->SetFont('Arial', 'B', 12);
        $pdf->SetTextColor($this->corPreto[0], $this->corPreto[1], $this->corPreto[2]);
        $pdf->SetXY($startX + 15, $startY + 15);
        $pdf->Cell($cardWidth - 30, 20, utf8_decode("TOTAL DE PRODUTOS"), 0, 1, 'L');
        $pdf->SetFont('Arial', 'B', 24);
        $pdf->SetTextColor($this->corPrimary[0], $this->corPrimary[1], $this->corPrimary[2]);
        ...
        $pdf->SetFont('Arial', 'B', 24);
        $pdf->SetTextColor($this->corAlerta[0], $this->corAlerta[1], $this->corAlerta[2]);
        ...
        $pdf->SetFont('Arial', 'B', 24);
        $pdf->SetTextColor($this->corSecondary[0], $this->corSecondary[1], $this->corSecondary[2]);
        $pdf->SetXY($startX + 2 * ($cardWidth + $cardMargin) + 15, $startY + 40);
        $pdf->Cell($cardWidth - 30, 25, $resumo['total_categorias'], 0, 1, 'L');
    
        // Table setup
        $consulta = "SELECT * FROM produtos $where ORDER BY natureza, nome_produto";
        ...
        $colunas = ['ID', 'Código', 'Produto', 'Quant.', 'Categoria'];
        $larguras = [
            round($larguraDisponivel * 0.06),
            round($larguraDisponivel * 0.18),
            round($larguraDisponivel * 0.44),
            round($larguraDisponivel * 0.12),
            round($larguraDisponivel * 0.20)
        ];
        if ($startDate && $endDate) {
            $colunas[] = 'Data Cadastro';
            $larguras = [
                round($larguraDisponivel * 0.06),
                round($larguraDisponivel * 0.18),
                round($larguraDisponivel * 0.34),
                round($larguraDisponivel * 0.12),
                round($larguraDisponivel * 0.20),
                round($larguraDisponivel * 0.10)
            ];
        }
    
        $pdf->SetXY($margemTabela, $startY + $cardHeight + 40);
        $pdf->SetFont('Arial', 'B', 11);
        $pdf->SetFillColor($this->corPrimary[0], $this->corPrimary[1], $this->corPrimary[2]);
        $pdf->SetTextColor($this->corBranco[0], $this->corBranco[1], $this->corBranco[2]);
        ...
        if ($result > 0) {
            foreach ($query as $idx => $row) {
                if ($categoriaAtual != $row['natureza'] && !$categoria) {
                    ...
                    $pdf->SetFont('Arial', 'B', 12);
                    $pdf->SetTextColor($this->corBranco[0], $this->corBranco[1], $this->corBranco[2]);
                    $pdf->SetFillColor($this->corSecondary[0], $this->corSecondary[1], $this->corSecondary[2]);
                ...
                if ($linhaAlternada) {
                    $pdf->SetFillColor($this->corCinzaClaro[0], $this->corCinzaClaro[1], $this->corCinzaClaro[2]);
                } else {
                    $pdf->SetFillColor($this->corBranco[0], $this->corBranco[1], $this->corBranco[2]);
                }
                ...
                if ($startDate && $endDate) {
                    ...
                    $pdf->SetXY($posX + 5, $y);
                    $pdf->Cell($larguras[5] - 10, $alturaLinhaDados, date('d/m/Y', strtotime($row['data_cadastro'])), 0, 0, 'C');
                }
            ...
            $pdf->SetTextColor($this->corTextoSubtil[0], $this->corTextoSubtil[1], $this->corTextoSubtil[2]);
            $pdf->SetFillColor(250, 250, 250);
            $pdf->RoundedRect($margemTabela, $y, array_sum($larguras), 40, 5, 'FD');
            $pdf->SetXY($margemTabela, $y + 12);
            $pdf->Cell(array_sum($larguras), 16, utf8_decode("Nenhum produto encontrado com os filtros aplicados"), 0, 1, 'C');
        }
    
        $this->addFooter($pdf);
        $pdf->Output("relatorio_personalizado.pdf", "I");
}
}
?>